...
void ObjModel::draw()
{
	...
	for(size_t i = 0; i < groups.size(); i++)
	{
		...
		auto material = materials[objGroup->materialIndex];
		material->texture->bind();
		glEnable(GL_TEXTURE_2D);

		glBegin(GL_TRIANGLES);
		for (std::list<Face>::iterator face = objGroup->faces.begin(); face != objGroup->faces.end(); ++face)
		{
			for (std::list<Vertex>::iterator vertex = face->vertices.begin(); vertex != face->vertices.end(); ++vertex)
			{
...
void ObjModel::loadMaterialFile( const std::string &fileName, const std::string &dirName )
{
	...
	while(!pFile.eof())
	{
		...
		else if(params[0] == "map_kd")
		{
			...
			currentMaterial->texture = new Texture("steve.png");
